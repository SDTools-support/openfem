...
%       $Revision: 1.93 $  $Date: 2021/09/15 17:08:48 $
...
{\tt ind=feutilb('AddNode -near epsl \tsi{value}',{\ti n1},{\ti n2});} returns a sparse matrix with non zero values in a given colum indicating of {\ti n1} nodes that are within {\tt epsl} of each {\ti n2} node (rows/columns correspond to {\tt n1/n2} node numbers). 
...
\ts{Hexa2Tetra} replaces each \hexah\ element by four \tetra\ elements (this is really not a smart thing to do).\\
\ts{Hexa2Penta} replaces each \hexah\ element by six \tetra\ elements (warning : this transformation may lead to incompatibilities on the triangular faces).\\
...
Command option \ts{-Replace} outputs the complete model on which selected elements have been refined. In this latter case, apparition of non conforming interfaces is possible.

Command option \ts{-MPC} allows generating MPC constraints (on DOF 1,2,3) at non-conforming interfaces to enforce displacement continuity. Generated MPC are named {\tt MPCedge} and {\tt MPCface} respectively concerning nodes added on edges and faces.
...
\end{verbatim}%enddoc
One can give a model subselection ({\tt FindElt} command string) as 2nd argument, to refine only a part of the model beams.
...
The \ts{RefineLine} generates line uniform refinements in the provided line segments, so that gaps between two points along the provided line are not higher than a given characteristic length. This is usefull for mesh seeding.
...
[model.Elt,RemovedElt]=feutil('RemoveElt \tsi{ElementSelectors}',model);

\noindent {\sl Element removal.} This function searches {\tt model.Elt} for elements which verify certain properties selected by \hyperlink{findelt}{\tsi{ElementSelectors}} and removes these elements from the model description matrix. 2nd output argument {\tt RemovedElt} is optional and contains removed elements.
...
{\tt model=feutil('}\ts{Renumber}{\tt',model,}\tsi{NewNodeNumbers}{\tt )} can be used to change the node numbers in the model. Currently nodes, elements, DOFs and deformations \begin{SDT}, nodeset, par, cyclic and other Case entries \end{SDT} are renumbered.

...
%       $Revision: 1.102 $  $Date: 2025/09/15 09:28:07 $
...
\item current to new basis : see \basis for the explanation of the three basis formats below
\begin{itemize}
\item \ts{bas} \tsi{BasID Type RelTo A1   A2   A3   B1 B2 B3 C1 C2 C3 0  0  0  s} 
\item \ts{bas} \tsi{BasID Type 0     NIdA NIdB NIdC 0  0  0  0  0  0  0  0  0  s} 
\item \ts{bas} \tsi{BasID Type 0     Ax Ay Az       Ux Uy Uz Vx Vy Vz Wx Wy Wz s} 
\end{itemize}
\item plane symmetry : 
\begin{itemize}
...
For each call, it is possible to either provide inputs as text string or as structure given on third argument with the field name corresponding to the desired transformation. 

A node selection can be provided in the text command (\ts{sel"NodeElt"}) or as a text in a {.sel} field of the {\tt RO} stucture to apply the transformation on only a part of the model. See \lttts{FindNode}.

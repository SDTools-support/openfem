...
%       $Revision: 1.122 $  $Date: 2026/03/17 09:27:22 $
...
feplot(model); % show model
MAP=feutil('getnormalElt MAP -dir1',model);fecom('showmap',MAP)
...
\rz{\htt{Coordm}} &  Identification number of the material coordinates system. {\bf Warning}  not implemented for all material formulations. \\
...
\rz{{\tt Form}}  &   1 volume force, 2 volume force proportional to density, 3 pressure, 13 surface based load (general direction), 4: fluid/structure coupling, see \fsc, 5 2D volume force, 6 2D pressure. 8 Wall impedance (acoustics), then uses the $R$ parameter in fluid (see~\ltr{m\_elastic}{2} and \eqr{feform_feacoustics_5} for matrix).\\
%
\end{tabular}

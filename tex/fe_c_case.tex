...
%       $Revision: 1.138 $  $Date: 2026/04/23 17:43:32 $
...
\ruic{fe\_case}{Get}{, \htr{fe\_case}{T}, \htr{fe\_case}{Set}, \htr{fe\_case}{Remove}, \htr{fe\_case}{Reset} ...} % - - 
\begin{itemize}
...
\htt{par} entries are used to define variable coefficients in element selections. Parameters follow the formats used in \ltr{fe\_range}{preRange}.

The base command \ts{ParAdd} allows quickly defining a parameter

{\tt model=fe\_case(model,'}\ts{ParAdd}\tsi{type nom min max scale}{\tt ',pname,par)}

Alternative are \ltr{upcom}{Par} and more detailed variants packaged in \ltr{fe\_caseg}{Par}. SDT-visc has specificity linked to the use of viscoelastic material properties~\cite{bal43}. 

Inputs defines the parameter as
\begin{itemize}
\item \tsi{type}, stored in {\tt par.coef(1)}, the types are associated to the \lts{fe\_mknl}{MatType} with tokens that refer to an internal numeric value
\begin{itemize}
\item \tsi{k} for stiffness (1) (see \lts{fe\_mknl}{MatType})
\item \tsi{m} for mass (2).
\item \tsi{c} for viscous damping (3.1).
\item \tsi{t} for shell thickness (3).
\item \tsi{ki} for hysteretic damping (4).
...
\item more types are available in generic parameters, see \ltr{fe\_range}{.param}.
\end{itemize}
\item {\tt pname} defines the parameter name.
\item {\tt par} provides the parameter definition as structure, string inputs will override the original values. At low level, the following fields are admissible
\begin{itemize}
\item {\tt .coef} a 5 value row vector providing the parameter values as {\tt [type nom min max scale]}, defined above. {\tt type} is used for assembly. The following values are overridden by \ferange\ if a more advanced definition is used.
...
\item {\tt .zCoef} (optional) defines a non standard weighting coefficient rule for the matrix.
...
{\tt model=fe\_case(model,'un=0','Normal motion',map);} where {\tt map} gives normal at nodes generates an {\tt mpc} case entry that enforces the condition $\ve{u}^T\ve{n}=0$ at each node of the map.

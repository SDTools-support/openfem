...
\rz{\tt data.dir} & used to define general surface traction force, combined with {\tt data.DOF=[.01;.02;.03]} and {\tt data.def} as magnitude. Note that the norm of the direction vector is accounted for in the force magnitude. This is currently only supported in 3D cases.
%
\end{tabular}
...
A general surface traction force applies a distributed surface load along a constant direction over the selection, as shown in the following example
%begindoc
\begin{verbatim}
% Applying a general surface traction load case in a model using sets
 model = femesh('testubeam plot');
 data=struct('sel','x==-.5', ... 
             'def',10,'DOF',[1:3]'/100,'dir',[-sqrt(2)/2 0 sqrt(2)/2]);
 model=fe_case(model,'Fsurf','Surface load',data);
 Load = fe_load(model); feplot(model,Load);
\end{verbatim}%enddoc
...
{\tt r1 = fe\_mat('SetMat {\ti MatId}  {\ti Param}={\ti value}',model)} can be used to set given parameter \tsi{Param} at the value \tsi{value} in the model properties. 
For example one can set density of matid 111 at 5000 as following\\
{\tt rho=fe\_mat('SetMat 111 rho=5000',model);}\\

{\tt PrePl} is a cell array with first row defining columns of acceptable material properties. Basic columns are {\tt MatId}, {\tt Type}, {\tt color}, {\tt name}, others are material dependent and listed using uing {\tt fe\_mat('info')}. 

\ruic{fe\_mat}{SetPro}{Pro} % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
{\tt PreIl} is a cell array with first row defining columns of acceptable element properties. For a list of accepted columns use {\tt fe\_mat('info')}. 
...
\item {\tt 100} volume load, {\tt 101} pressure load, {\tt 102} inertia load, {\tt 103} initial stress load, {\tt 106} general distributed surface load. Note that some load types are only supported with the {\tt mat\_og} element family; 
...
This command is used to build the \ltt{InfoAtNode} entry from the stack entry \ltt{EltOrient} containing a material orientation map before step 5 of the {\tt init} command. The preferred mechanism is to define a material map associated to an element property as illustrated in \ser{VectFromDir}. 
